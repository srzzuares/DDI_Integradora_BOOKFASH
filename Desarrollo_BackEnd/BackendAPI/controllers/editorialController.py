...
def get_One(ideditorial):
    listOneEdit = conn.execute(editorial.select().where(
        editorial.c.id_editorial == ideditorial)).first()
...
def get_update(data, ideditorial):
    ...
    if ip.get("status") == "No existe el editorial":
        ...
        result = conn.execute(editorial.update().values(
            dict(data)).where(editorial.c.id_editorial == ideditorial))
        conn.commit()
...
def get_delete(ideditorial):
    ...
    if ip.get("status") == "No existe el editorial":
        ...
        result = conn.execute(editorial.delete().where(
            editorial.c.id_editorial == ideditorial))
        conn.commit()
