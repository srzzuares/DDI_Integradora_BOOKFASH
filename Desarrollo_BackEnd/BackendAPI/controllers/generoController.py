...
def get_One(idgenero):
    listOneGene = conn.execute(genero.select().where(
        genero.c.id_genero == idgenero)).first()
...
def get_update(data, idgenero):
    ...
    if ip.get("status") == "No existe el genero":
        ...
        result = conn.execute(genero.update().values(
            dict(data)).where(genero.c.id_genero == idgenero))
        conn.commit()
...
def get_delete(idgenero):
    ...
    if ip.get("status") == "No existe el genero":
        ...
        result = conn.execute(genero.delete().where(
            genero.c.id_genero == idgenero))
        conn.commit()
